...
- (void)bulidNewKlineModelWithNewPrice:(KlineModel *)newKlineM timestamp:(NSInteger)timestamp volumn:(NSNumber *)volumn dataArray:(NSMutableArray<KlineModel *> *)dataArray isFakeData:(BOOL)isFakeData
{
    ...
    if ((timestamp>=lastKlineModel.timestamp&&timestamp<lastKlineModel.timestamp+shouldAddTimestamp)) {
        ...
        newKlineMlodel.closePrice = newKlineM.closePrice;
      
        if (newKlineM.highestPrice>lastKlineModel.highestPrice) {
          
            newKlineMlodel.highestPrice = newKlineM.highestPrice;
        }else{
          
            newKlineMlodel.highestPrice = lastKlineModel.highestPrice;
        }
        if (newKlineM.lowestPrice<lastKlineModel.lowestPrice) {
            newKlineMlodel.lowestPrice = newKlineM.lowestPrice;
        }else{
    ...
    }else
    {
        ...
        newKlineMlodel.openPrice = newKlineM.openPrice;
        newKlineMlodel.closePrice = newKlineM.closePrice;
        newKlineMlodel.highestPrice = newKlineM.highestPrice;
        newKlineMlodel.lowestPrice = newKlineM.lowestPrice;
        newKlineMlodel.isFakeData = isFakeData;
        //TODO  成交量最新的没有返回
        newKlineMlodel.volumn = newKlineM.volumn;
