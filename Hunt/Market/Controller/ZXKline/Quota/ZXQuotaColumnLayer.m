...
- (NSArray *)drawQuotaWithIndex:(NSInteger)index xPosition:(CGFloat)x
{
    ...
    if (self.quotaMinValue<0&&self.quotaMaxValue>0&&!self.columnColorArr) {
        ...
        if (x>zeroValue) {
            strokeColor = [[SEUserDefaults shareInstance] getRiseOrFallColor:RoseType].CGColor;
        }else{
            strokeColor = [[SEUserDefaults shareInstance] getRiseOrFallColor:FallType].CGColor;
        }
    }else{
